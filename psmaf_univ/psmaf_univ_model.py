...
class PSMAFUNIVModel(nn.Module):
    ...
    Structured encoder metadata is preserved for the task adapter. In particular,
    BNC features may describe their layout with either ``spatial_shape`` or
    ``grid_size``; the aliases must agree when both are present. Square-grid
    inference remains disabled by default because it can corrupt rectangular
    feature layouts whose token count happens to be a perfect square.
    """
    ...
    def forward(
        ...
        *,
        ...
        for item in encoded:
            ...
            # Retain layout and diagnostic metadata supplied by the encoder rather
            # than reconstructing a lossy adapter input dictionary.
            adapter_item = dict(item)
            adapter_item.setdefault("tensor", tensor)
            adapter_features.append(adapter_item)
        try:
            ...
            if "require spatial_shape" in str(error):
                ...
                raise ValueError(
                    f"UNIV encoder returned BNC tokens without layout metadata; include spatial_shape "
                    f"or grid_size "
